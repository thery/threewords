...
| **`discrepancies.md`** | **Start here for the findings.** Every point where the machine-checked proof departs from `paper3.pdf`, written in the paper's own notation and readable without any proof-assistant background: four amendments (Algorithm 7's inner test, Algorithm 18's two `Fast2Sum` calls, `δ₂`'s `u⁴` term, Lemma 1's hypotheses), three proofs whose statements are nevertheless correct, and one bound where ours is the loose one. |
