...
| **`discrepancies.pdf`** | **The note to send to the authors** — `discrepancies.md` typeset in the paper's own notation (source `discrepancies.tex`, rebuilt by `scripts/make-discrepancies-pdf.sh`). |
| **`discrepancies.md`** | **Start here for the findings.** Every point where the machine-checked proof departs from `paper3.pdf`, written in the paper's own notation and readable without any proof-assistant background: four amendments (Algorithm 7's inner test, Algorithm 18's two `Fast2Sum` calls, `δ₂`'s `u⁴` term, Lemma 1's hypotheses), three proofs whose statements are nevertheless correct, one bound where ours is the loose one, and a reading of the NXP reference implementation (which carries the `Fast2Sum` defect unrepaired, and whose own comments report two further errors in the appendix algorithms). |
...
its Rocq definition/theorem. **Algorithms 1–15, 18 and 20 and Theorems 1–11 are
all proved, with zero admits.** Full catalogue with lemma names in
`doc/paper3-map.md`; one write-up per theorem in `doc/thm1.md` … `doc/thm11.md`;
and the departures from the paper are collected in `doc/discrepancies.md`.
...
| 2 | [2Sum](https://github.com/thery/threewords/blob/main/code/coq/TwoSum.v#L67) | [correctness](https://github.com/thery/threewords/blob/main/code/coq/TwoSum.v#L495) | ✅ |
...
| 4 | [VecSum](https://github.com/thery/threewords/blob/main/code/coq/VecSum.v#L110) | [Thm 1](https://github.com/thery/threewords/blob/main/code/coq/VecSum.v#L892) + [Cor 1](https://github.com/thery/threewords/blob/main/code/coq/VecSum.v#L1041) | ✅ |
...
|  | keep-first-`k` error | [Thm 3](https://github.com/thery/threewords/blob/main/code/coq/Nonoverlap.v#L766) | ✅ |
...
| 8 | [TWSum](https://github.com/thery/threewords/blob/main/code/coq/TWSum.v#L1209) | [Thm 6](https://github.com/thery/threewords/blob/main/code/coq/Thm6.v#L4153) + [error](https://github.com/thery/threewords/blob/main/code/coq/TWSum.v#L1319) | ✅ |
| 9 | [3Prod^acc (TW×TW)](https://github.com/thery/threewords/blob/main/code/coq/ThreeProd.v#L93) | [Thm 7](https://github.com/thery/threewords/blob/main/code/coq/ThreeProd.v#L4085) + [error](https://github.com/thery/threewords/blob/main/code/coq/ThreeProd.v#L4131) | ✅ |
...
| 13 | [3Reci](https://github.com/thery/threewords/blob/main/code/coq/ThreeReci.v#L180) | [isTW](https://github.com/thery/threewords/blob/main/code/coq/ThreeReci.v#L1437) + [Thm 9 acc](https://github.com/thery/threewords/blob/main/code/coq/ThreeReci.v#L1693) + [Thm 9 fast](https://github.com/thery/threewords/blob/main/code/coq/ThreeReci.v#L1726) | ✅ |
...
| 15 | [3SqRt](https://github.com/thery/threewords/blob/main/code/coq/ThreeSqRt.v#L286) | [isTW](https://github.com/thery/threewords/blob/main/code/coq/ThreeSqRt.v#L1588) + [Thm 11 acc](https://github.com/thery/threewords/blob/main/code/coq/ThreeSqRt.v#L2220) + [Thm 11 fast](https://github.com/thery/threewords/blob/main/code/coq/ThreeSqRt.v#L2271) | ✅ |
| 18 | [3Prod (DW×TW, head-1)](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L159) | [isTW](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L314) + [δ₂](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L458) | ✅ |
| 20 | [3Prod (TW×TW, head-1)](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L731) | [isTW](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L760) + [δ₃](https://github.com/thery/threewords/blob/main/code/coq/ThreeProdOne.v#L1108) | ✅ |
...
| `ThreeProdOne.v` | Algorithms 18 and 20 (3Prodᵒⁿᵉ), the product of a double word (Alg 18) resp. a **triple** word (Alg 20) by a triple word whose leading limb is exactly `1`. Its two correctness results — `ThreeProdOne_isTW` and `ThreeProdOne_error` (relative error `≤ u³ + 620u⁴`, an order of magnitude sharper than Theorem 8's `10.5u³`) — both proved; this is the second product of Algorithms 13 and 14, and the sharper bound is what makes Theorem 9's `11.5 = 10.5 + 1`. The paper announces `u³ + 260u⁴`; the `u³` is exact, the `u⁴` is ours. Algorithm 20's bound is `ThreeProdOneTW_error_c`, `δ₃(c) = 6u³ + (31c + 10)u⁴` for a second factor within `cu²` of `1` (`40 ≤ c ≤ 112`), with `ThreeProdOneTW_error` its `c = 40` instance, `6u³ + 1250u⁴`: Algorithm 14 supplies `c = 40`, Algorithm 15 can only supply `c = 105`. One deviation from the paper is necessary: its `Fast2Sum(b₁, z₀₁⁺)` is unjustified and loses `~32u³` on legal inputs (`doc/alg18_fast2sum_bug.py`); repaired at no operation cost by sorting the two arguments. |
...
| `ThreeDiv.v` | Algorithm 14 (3Div), the **quotient** of two triple words (`p ≥ 10`) — Algorithm 13 with the dividend folded in, so that the two products `z·b` and `2 − b·x` become independent. Its four results — `ThreeDiv_isTW`, `ThreeDivFast_isTW`, `ThreeDiv_error` (**paper Theorem 10**, relative error `≤ 27u³ + 2500u⁴`) and `ThreeDivFast_error` (`≤ 42u³ + 2575u⁴`) — all proved; see `doc/thm10.md`. Our `27u³` is above the paper's `24u³`, because our `δ₃` is `6u³ + 1250u⁴` where the paper asserts `3u³ + 264u⁴`: numerically the paper is **not** refuted, our bound is simply loose (`doc/alg20_delta3_search.py`). |
| `ThreeSqRt.v` | Algorithm 15 (3SqRt), the **square root** of a triple word by one Newton step on `r ← r(3/2 − ½r²x)` (`p ≥ 11`). All four results proved: `ThreeSqRt_isTW`, `ThreeSqRtFast_isTW`, `ThreeSqRt_error` (**paper Theorem 11**, `≤ 24u³ + 10260u⁴`) and `ThreeSqRtFast_error` (`≤ 39u³ + 10333u⁴`) — **the paper's own constants**, the only theorem here where that survives every intermediate constant of ours being worse. Our route is not the supplementary's (`doc/Algorithms_for_Triple-Word_Arithmetic.pdf` §3; `doc/old-triplewors.pdf` never wrote its Section 10 — its roadmap literally reads "Section 10 [???]"): the supplementary's weight `1.5` on `δ₁` discards a cancellation between the two occurrences of `i⁽¹⁾`, and keeping it makes the weight `0.5`. That is what buys the `u³` room absorbing our `δ₃ = 6u³ + 3265u⁴`. See `doc/thm11.md`. |
