...
## §6–10 Multiplication, division, square root — Alg 9–13 done
...
| 14 | 3Div(z,x) (119, 4 tests) — quotient | **Thm 10** (err `24u³+1509u⁴`, `p≥10`; honestly `1510u⁴` at `p=10`) | 🚧 `ThreeDiv.v`: everything PROVED — `ThreeDiv_isTW`/`ThreeDivFast_isTW`, `div_error_core`, `ThreeDiv_error` (`24u³+1510u⁴`) / `ThreeDivFast_error` (`39u³+1583u⁴`) — modulo Alg 20's `δ₃` (`doc/thm10.md`) |
...
**Formalised: the DW×TW product (Alg 11 / Thm 8).** `ThreeProdDW_isTW` and
`ThreeProdDW_error` (`10.5u³+39u⁴`), zero admits (`doc/thm8.md`). Algorithm 11
is Algorithm 9 with `x₂ = 0`, so correctness is inherited from Theorem 7; the
