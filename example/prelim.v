...
(* By rewriting *)
Lemma test a b c d : (a + b) * (c + d) = a * c + a * d + b * c + b * d.
Proof.
...
(* Plus in a cal mode *)
Lemma test1 a b c d : (a + b) * (c + d) = a * c + a * d + b * c + b * d.
Proof.
(* 
refine (eq_trans (_ : _ = (a + b) * c + ?[toto]) _).
refine(_ : (a + b) * (c + d) = (a + b) * c + 3).
*)
apply eq_trans with (y := (a + b) * c + (a + b) * d).
  now apply mul_add_distr_l.
apply eq_trans with (y := a * c + b * c + (a + b) * d).
  apply f_equal2 with (f := plus).
    now apply mul_add_distr_r.
    trivial.
apply eq_trans with (y := a * c + b * c + a * d + b * d).
  apply eq_trans with (y := a * c + b * c + (a * d + b * d)).
    apply f_equal2 with (f := plus).
    trivial.
    now apply mul_add_distr_r.
...
apply eq_trans with (y := a * c + (b * c + a * d) + b * d).
  ...
  apply sym_equal.
  now apply add_assoc.
apply eq_trans with (y := a * c + (a * d + b * c) + b * d).
  apply f_equal2 with (f := plus).
  2: trivial.
  apply f_equal2 with (f := plus).
  1: trivial.
